...
function buildMockSnapshot(company?: string | null): DashboardStateSnapshot {
  ...
  return {
    ...
    metrics: [
      { id: 'renewals', title: 'Applications awaiting decision', value: formatCount(renewals), delta: `${seededValue(seed + 107, 18, 74)} urgent`, trend: 'up' },
      { id: 'inspections', title: 'Inspections overdue', value: formatCount(inspectionsOverdue), delta: `${countiesFlagged} counties`, trend: inspectionsOverdue > 30 ? 'down' : 'neutral' },
      { id: 'alerts', title: 'Escalations open', value: formatCount(escalations), delta: `${seededValue(seed + 109, 1, 9)} new today`, trend: 'up' },
      { id: 'discipline', title: 'Disciplinary cases open', value: formatCount(disciplineCases), delta: `${seededValue(seed + 113, 4, 18)} hearing-ready`, trend: 'neutral' },
    ],
    priorityQueues: [
      {
